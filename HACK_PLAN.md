...
| **Whitney** (Goldenrod City) | Normal | Bulky pivot / status-stall around a hard-hitting wallbreaker — Miltank left untouched as the signature threat (Rollout/Attract/Stomp/Milk Drink), Clefairy rebuilt into real support (Sing/Charm/Encore/Disarming Voice, all naturally legal by Lv17) — **done**, see `data/Trainers.c` trainer 30 | Goldenrod's surrounding farmland (Route 34 / National Park edge) fits Normal-type livestock/common-critter ecology well already | Fighting needed (Normal's only weakness). Closed via Mankey added to Route 34 (see `documentation/areas/Route-34.md`) — **done**. |
| **Morty** (Ecruteak City) | Ghost | Status-stall trapping core: Gastly leads with Mean Look + Will-O-Wisp (trap, then burn) backed by Curse/Lick, sleep-stalling Haunter and a Sucker-Punch Haunter hold the middle, Gengar (unchanged Hypnosis/Shadow Ball/Mean Look/Sucker Punch) closes as the ace — canon roster has no natural Trick Room user worth building around (all three are fast, not slow), so the archetype landed on the literal "Will-O-Wisp burn stall, trapping" half of the original plan text rather than Trick Room — **done**, see `data/Trainers.c` trainer 31 | Burned Tower / old-town lore is already a strong ghost biome, no change needed | Dark needed (Ghost's other weakness besides Ghost itself). The plan's assumption that Murkrow already spawns near National Park at night didn't hold in this codebase's tables (`grep` showed it only on Kanto's Route 7/16) — closed via Murkrow added to Route 36's night pool (see `documentation/areas/Route-36.md`) — **done**. |
